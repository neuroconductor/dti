...
dtiTensorneu <- function(object,  ...) cat("No DTI tensor calculation defined for this class:",class(object),"\n")

setGeneric("dtiTensorneu", function(object,  ...) standardGeneric("dtiTensorneu"))

setMethod( "dtiTensorneu", "dtiData",
           function( object, 
...
##  use ms0 and mbv to rescale parameters such that they are of comparable magnitude
               ...
               if(mc.cores==1){
                 ...
                 param <- matrix(.C("dtens",
                         ...
                         as.double(object@sdcoef),
                         as.double(rep(0,ngrad)),#si
                         as.double(rep(0,ngrad)),#var                         
                 ...
                 x[1:7,] <- param
                 x[-(1:7),] <- matrix(z$si,c(ngrad,ntotal))[,mask]
                 param <- plmatrix(x,ptensnl,ngrad=ngrad,btb=object@btb/mbv,
                                  sdcoef=object@sdcoef,maxit=1000,reltol=1e-7)
                 }
                 ...
                 z <- .Fortran("tensres",
                               ...
                               rss = double(nvox),
                               PACKAGE="dti",DUP=FALSE)[c("res","rss")]
                 res <- matrix(0,ngrad,ntotal)
                 res[,mask] <- z$res
                 rss <- numeric(ntotal)
                 rss[mask] <- z$rss
