...
setMethod("dti.smooth", "dtiData", function(object,hmax=5,hinit=NULL,lambda=20,
                                            ...
                                            minanindex=NULL,hsig=2.5,lseq=NULL, method="nonlinear",varmethod="residuals",rician=TRUE,niter=5,varmodel="local",volseq=TRUE) {
switch(method,"linear" = dtilin.smooth(object,hmax,hinit,lambda,rho,graph,slice,quant,minanindex,hsig,lseq,varmethod,varmodel),
              "nonlinear" =  dtireg.smooth(object,hmax,hinit,lambda,rho,graph,slice,quant,minanindex,hsig,lseq,varmethod,rician,niter,varmodel,volseq=volseq))
}
)
...
rdtianiso <- function(dtobject,hmax=5,lambda=20,rho=1,graph=FALSE,slice=NULL,quant=.8,minanindex=NULL){
  ...
  z <- .Fortran("projdt",
                ...
                DUP=FALSE,
                PACKAGE="dti")[c("D","anindex","andirection","det","mask")]
  y <- array(z$D,dimy)
  z$bi <- 1/sigma2
  dim(z$D) <- dimy
  ...
  while( hakt <= hmax) {
    if (any(h0 >= 0.25)) {
     ...
     z <- .Fortran("rawsdti",
                ...
                DUP=FALSE,
                PACKAGE="dti")[c("D","bi","anindex","andirection","det","mask")]
...
#z <- list(D=z$D,bi=z$bi,anindex=z$anindex,andirection=z$andirection,mask=z$mask, ddim0=ddim0,xind=xind,yind=yind,zind=zind,InvCov=Bcov,call=args)
#class(z) <- "dti"
