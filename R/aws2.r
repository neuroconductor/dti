...
#        z$sigma2hat[z$sigma2hat>4*maxsigma2] <- 4*maxsigma2
  ...
  invisible(new("dtiTensor",
                list(D = z$D, th0= z$th0, Varth= z$Varth, sigma = z$sigma2hat, scorr = scorr, s0hat = z$th0, bw = dtobject$bw, hmax = hmax, mask = mask, s2rician=if(rician) z$sigma2r else NULL, ni=z$bi*if(wlse) z$sigma2hat else 1),
                btb   = btb,
...
#
#   No the regularized version according to Koay et. al. (2006)
#   this is also based on an alternative statistical penalty
#   (using log-likelihood differences)
#
