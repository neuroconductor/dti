datamixtens <- function(mtobj,grad,sigma,bf=1,smoothS0=TRUE,maxs0=10000){
#
...
ngrad <- ngrad + ns0
dim(s0) <- ds0
if(max(s0) < maxs0) s0 <- s0/max(s0)*maxs0
...
sb <- sqrt(rnorm(sb,sb,sigma)^2+rnorm(sb,0,sigma)^2)
dim(sb) <- c(ddim,ngrad)
ddim0 <- ddim
