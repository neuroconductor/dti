...
zt <- dtiTensor(z)
zmix <- dwiMixtensorpl0(z,optmethod="Nelder-Mead",isocomp=FALSE,maxcomp=maxcomp)
...
size <- 500
w1 <- show3d(z0,scale=.5,FOV=1,windowRect = c(1, 1, size, size))
w2 <- show3d(zt,what="odf",minalpha=1,falevel=0,scale=.5,FOV=1,windowRect = c(size+11, 1, 2*size+10, size))
w3 <- show3d(zmix,scale=.5,FOV=1,windowRect = c(1, size+11 , size, 2*size+10))
w4 <- show3d(zqball,scale=.5,FOV=1,windowRect = c(size+11, size+11, 2*size+10, 2*size+10))
